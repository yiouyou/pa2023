...
Learn more about [iostat](https://linux.die.net/man/1/iostat) and [PerfMon](/en-us/windows/win32/perfctrs/performance-counters-portal).
...
For more information on VM sizes and on the IOPS, throughput, and latency available for each type of VM, see [Sizes for virtual machines in Azure](sizes).
...
| **Performance factors** | Â  | Â  | Â  |
| **IO size** | Smaller IO size yields higher IOPS. | Larger IO size to yields higher Throughput. | Â  |
...
| **Disk Caching** | Enable ReadOnly Cache on premium storage disks with Read heavy operations to get higher Read IOPS. | Â  | Enable ReadOnly Cache on premium storage disks with Read heavy operations to get very low Read latencies. |
| **Disk Striping** | Use multiple disks and stripe them together to get a combined higher IOPS and Throughput limit. The combined limit per VM should be higher than the combined limits of attached premium disks. | Â  | Â  |
| **Stripe Size** | Smaller stripe size for random small IO pattern seen in OLTP applications. For example, use stripe size of 64 KB for SQL Server OLTP application. | Larger stripe size for sequential large IO pattern seen in Data Warehouse applications. For example, use 256 KB stripe size for SQL Server Data warehouse application. | Â  |
| **Multithreading** | Use multithreading to push higher number of requests to Premium Storage that will lead to higher IOPS and Throughput. For example, on SQL Server set a high MAXDOP value to allocate more CPUs to SQL Server. | Â  | Â  |
