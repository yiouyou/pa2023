...
You can view the available VM SKUs with the [Get-AzComputeResourceSku](/en-us/powershell/module/az.compute/get-azcomputeresourcesku) command. The following example lists available VM SKUs in the *eastus2* region:
...
Create an Azure resource group with [New-AzResourceGroup](/en-us/powershell/module/az.resources/new-azresourcegroup). A resource group is a logical container into which Azure resources are deployed and managed. In this example, a resource group named *myResourceGroup* is created in the *eastus2* region.
...
Create a network card with [New-AzNetworkInterface](/en-us/powershell/module/az.network/new-aznetworkinterface) for the virtual machine. The network card connects the virtual machine to a subnet, network security group, and public IP address.
...
Create the virtual machine with [New-AzVM](/en-us/powershell/module/az.compute/new-azvm).
...
You created the VM's IP address resource in the same availability zone as the VM. The managed disk resource for the VM is created in the same availability zone. You can verify this with [Get-AzDisk](/en-us/powershell/module/az.compute/get-azdisk):
