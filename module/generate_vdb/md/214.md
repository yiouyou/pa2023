...
2. Renames the VM based off of the user-supplied VM name by pulling this data from [Azure Instance Metadata Service (IMDS)](instance-metadata-service). **Note** IMDS also provides other [instance metadata](instance-metadata-service#access-azure-instance-metadata-service), such as SSH Public Keys, so you can set more than the hostname.
...
For more information, see [Linux provisioning](provisioning).
