...
If you need to manage Classic virtual machines, see [Using the VMAccess extension](/en-us/previous-versions/azure/virtual-machines/linux/classic/reset-access-classic).
...
The following examples use [az vm user](/en-us/cli/azure/vm/user) commands. To perform these steps, you need to [install the latest Azure CLI](/en-us/cli/azure/install-az-cli2) and sign in to an Azure account by using [az login](/en-us/cli/azure/reference-index).
...
The [`az vm user update` command](/en-us/cli/azure/vm) appends the new public key text to the `~/.ssh/authorized_keys` file for the admin user on the VM. This command doesn't replace or remove any existing SSH keys. This command doesn't remove prior keys set at deployment time or subsequent updates by using the VMAccess extension.
...
The following examples use raw JSON files. Use the [az vm extension set](/en-us/cli/azure/vm/extension#az-vm-extension-set) command to then call your JSON files. Azure templates can also call these JSON files.
