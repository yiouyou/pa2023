...
* Ensure that VM Image Builder has the correct permissions to read/write images and to connect to the storage account. For more information, review the permissions documentation for the [Azure CLI](image-builder-permissions-cli) or [Azure PowerShell](image-builder-permissions-powershell).
* VM Image Builder will fail the build if the scripts or inline commands fail with errors (non-zero exit codes). Ensure that you've tested the custom scripts and verified that they run without error (exit code 0) or require user input. For more information, see [Create an Azure Virtual Desktop image by using VM Image Builder and PowerShell](../windows/image-builder-virtual-desktop#tips-for-building-windows-images).
...
For PowerShell, you'll need to install the [VM Image Builder PowerShell modules](../windows/image-builder-powershell#prerequisites).
...
There are cases where [Managed Service Identities (MSI)](/en-us/azure/virtual-machines/linux/image-builder-permissions-cli#create-a-user-assigned-managed-identity) assigned to the image template cannot be used:

* The Image Builder template uses a customer provided staging resource group and the MSI is deleted before the image template is deleted ([staging resource group](image-builder-json#properties-stagingresourcegroup) scenario)
* The created [Managed Service Identities (MSI)](image-builder-permissions-cli#create-a-user-assigned-managed-identity) assigned to the image template cannot be used

#### Solution

Use Azure CLI to reset identity on the image template. Ensure you [update](/en-us/cli/azure/update-azure-cli) Azure CLI to the 2.45.0 version or later.
...
For more information about configuring permissions, see [Configure VM Image Builder permissions by using the Azure CLI](image-builder-permissions-cli) or [Configure VM Image Builder permissions by using PowerShell](image-builder-permissions-powershell).
...
To implement this solution in portal, follow the instructions in this documentation: [Assign Azure roles using the Azure portal - Azure RBAC](../../role-based-access-control/role-assignments-portal).

For [Step 1: Identify the needed scope](../../role-based-access-control/role-assignments-portal#step-1-identify-the-needed-scope): The needed scope is your resource group.

For [Step 3: Select the appropriate role](../../role-based-access-control/role-assignments-portal#step-3-select-the-appropriate-role): The role is Contributor.

For [Step 4: Select who needs access](../../role-based-access-control/role-assignments-portal#step-4-select-who-needs-access):â¯Select member âAzure Virtual Machine Image Builderâ

Then proceed to [Step 6: Assign role](../../role-based-access-control/role-assignments-portal#step-6-assign-role) to assign the role.
...
The storage account name uses the pattern IT\_<ImageResourceGroupName>*<TemplateName>*<GUID> (for example, *IT\_aibmdi\_helloImageTemplateLinux01*).
...
1. *Deploy the build VM and dependencies by using ARM templates to the IT\_ staging resource group* stage. This stage includes multiple POSTs to the VM Image Builder resource provider:
...
5. If you expect that the customizations need more time, increase the value of [buildTimeoutInMinutes](image-builder-json). The default is 4 hours.
6. If you have resource-intensive actions, such as downloading gigabytes (GB) of files, consider the underlying build VM size. The service uses a Standard\_D1\_v2 VM. The VM has 1 vCPU and 3.5 GB of memory. If you're downloading 50 GB, you'll likely exhaust the VM resources and cause communication failures between VM Image Builder and the build VM. Retry the build with a larger-memory VM by setting the [VM\_size](image-builder-json#vmprofile).
...
Review your scripts for firewall changes or enablement, or changes to SSH or WinRM, and ensure that any changes allow for constant connectivity between the service and the build VM on the previously mentioned ports. For more information, see [VM Image Builder networking options](image-builder-networking).
...
As described in [Create an VM Image Builder template](image-builder-json), the time-out must be set to 0 to use the default or set to more than 5 minutes to override the default. Change the time-out in your template to 0 to use the default or to a minimum of 6 minutes.
...
1. Update the inline command by adding **Write-Host / Echo âSleepâ**. This gives you time to search in the log.
2. Add a `sleep` value of at least 10 minutes by using a [Start-Sleep](/en-us/powershell/module/microsoft.powershell.utility/start-sleep) or `Sleep` Linux command.
...
For more information about Azure DevOps capabilities and limitations, see [Microsoft-hosted agents](/en-us/azure/devops/pipelines/agents/hosted#capabilities-and-limitations).
...
2. Add the [/mode:vm](/en-us/windows-hardware/manufacture/desktop/sysprep-command-line-options) option to the default `Sysprep` that VM Image Builder uses. For more information, go to the ["Override the commands"](#override-the-commands) section under "VMs created from VM Image Builder images aren't created successfully."
...
For more information, see [VM Image Builder overview](../image-builder-overview).
