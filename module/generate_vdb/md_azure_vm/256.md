...
Create a spotVM using [New-AzVmConfig](/en-us/powershell/module/az.compute/new-azvmconfig) to create the configuration. Include `-Priority Spot` and set `-MaxPrice` to either:
...
From a computer that has the Az PowerShell module installed (like your local machine), simulate an eviction using [Set-AzVM](/en-us/powershell/module/az.compute/set-azvm). Replace the resource group name and VM name with your own.
...
You can also create an Azure Spot Virtual Machine using the [Azure CLI](../linux/spot-cli), [portal](../spot-portal) or a [template](../linux/spot-template).

Query current pricing information using the [Azure retail prices API](/en-us/rest/api/cost-management/retail-prices/azure-retail-prices) for information about Azure Spot Virtual Machine pricing. The `meterName` and `skuName` will both contain `Spot`.
