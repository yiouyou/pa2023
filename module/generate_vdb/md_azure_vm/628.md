...
Have your HTTP clients bypass web proxies within the VM when querying IMDS, and treat `169.254.169.254` the same as [`168.63.129.16`](../virtual-network/what-is-ip-address-168-63-129-16).

## Usage

### Access Azure Instance Metadata Service

To access IMDS, create a VM from [Azure Resource Manager](/en-us/rest/api/resources/) or the [Azure portal](https://portal.azure.com), and use the following samples.
...
| `evictionPolicy` | Sets how a [Spot VM](spot-vms) will be evicted. | 2020-12-01 |
...
| `placementGroupId` | [Placement Group](../virtual-machine-scale-sets/virtual-machine-scale-sets-placement-groups) of your scale set | 2017-08-01 |
| `plan` | [Plan](/en-us/rest/api/compute/virtualmachines/createorupdate#plan) containing name, product, and publisher for a VM if it's an Azure Marketplace Image | 2018-04-02 |
| `platformUpdateDomain` | [Update domain](availability) the VM is running in | 2017-04-02 |
| `platformFaultDomain` | [Fault domain](availability) the VM is running in | 2017-04-02 |
| `platformSubFaultDomain` | Sub fault domain the VM is running in, if applicable. | 2021-10-01 |
| `priority` | Priority of the VM. Refer to [Spot VMs](spot-vms) for more information | 2020-12-01 |
| `provider` | Provider of the VM | 2018-10-01 |
| `publicKeys` | [Collection of Public Keys](/en-us/rest/api/compute/virtualmachines/createorupdate#sshpublickey) assigned to the VM and paths | 2018-04-02 |
| `publisher` | Publisher of the VM image | 2017-04-02 |
| `resourceGroupName` | [Resource group](../azure-resource-manager/management/overview) for your Virtual Machine | 2017-08-01 |
| `resourceId` | The [fully qualified](/en-us/rest/api/resources/resources/getbyid) ID of the resource | 2019-03-11 |
...
| `securityProfile.encryptionAtHost` | Identifies if [Encryption at Host](disks-enable-host-based-encryption-portal) is enabled on the VM | 2021-11-01 |
| `securityProfile.securityType` | Identifies if the VM is a [Trusted VM](trusted-launch) or a [Confidential VM](../confidential-computing/confidential-vm-overview) | 2021-12-13 |
| `storageProfile` | See Storage Profile below | 2019-06-01 |
| `subscriptionId` | Azure subscription for the Virtual Machine | 2017-08-01 |
| `tags` | [Tags](../azure-resource-manager/management/tag-resources) for your Virtual Machine | 2017-08-01 |
...
| `virtualMachineScaleSet.id` | ID of the [Virtual Machine Scale Set created with flexible orchestration](flexible-virtual-machine-scale-sets) the Virtual Machine is part of. This field isn't available for Virtual Machine Scale Sets created with uniform orchestration. | 2021-03-01 |
...
| `vmScaleSetName` | [Virtual Machine Scale Set Name](../virtual-machine-scale-sets/overview) of your scale set | 2017-12-01 |
| `vmSize` | [VM size](sizes) | 2017-04-02 |
| `zone` | [Availability Zone](../availability-zones/az-overview) of your virtual machine | 2017-12-01 |
...
| `isSharedDisk`â â  | Identifies if the disk is shared between resources | 2021-05-01 |
...
â â  These fields are only populated for Ultra Disks; they are empty strings from non-Ultra Disks.
...
The resource disk object contains the size of the [Local Temp Disk](managed-disks-overview#temporary-disk) attached to the VM, if it has one, in kilobytes.
If there's [no local temp disk for the VM](azure-vms-no-temp-disk), this value is 0.
...
When creating a new VM, you can specify a set of data to be used during or after the VM provision, and retrieve it through IMDS. Check the end to end user data experience [here](user-data).
...
* [Windows](#tabpanel_4_windows)
* [Linux](#tabpanel_4_linux)
...
You can also use [Availability Zones](../availability-zones/az-overview) for the instances to make these decisions.
You can query this data directly via IMDS.

**Request**

* [Windows](#tabpanel_5_windows)
* [Linux](#tabpanel_5_linux)
...
* [Windows](#tabpanel_6_windows)
* [Linux](#tabpanel_6_linux)
...
* [Windows](#tabpanel_7_windows)
* [Linux](#tabpanel_7_linux)
...
* [Windows](#tabpanel_8_windows)
* [Linux](#tabpanel_8_linux)
...
* [Windows](#tabpanel_9_windows)
* [Linux](#tabpanel_9_linux)
...
* [Windows](#tabpanel_10_windows)
* [Linux](#tabpanel_10_linux)
...
* [Windows](#tabpanel_11_windows)
* [Linux](#tabpanel_11_linux)
...
* [Windows](#tabpanel_12_windows)
* [Linux](#tabpanel_12_linux)
...
* [Windows](#tabpanel_13_windows)
* [Linux](#tabpanel_13_linux)
...
* If you're looking to retrieve IMDS information for **Standard** SKU Public IP address, review [Load Balancer Metadata API](../load-balancer/howto-load-balancer-imds?tabs=windows) for more infomration.
...
| `plan` | The [Azure Marketplace Image plan](/en-us/rest/api/compute/virtualmachines/createorupdate#plan). Contains the plan ID (name), product image or offer (product), and publisher ID (publisher). | 2018-10-01 |
...
* [Windows](#tabpanel_14_windows)
* [Linux](#tabpanel_14_linux)
...
You can find the intermediate certificates on [this page](../security/fundamentals/azure-ca-details). The intermediate certificates for each of the regions can be different.
...
For detailed steps to enable this feature, see [Acquire an access token](../active-directory/managed-identities-azure-resources/how-to-use-vm-token).

## Load Balancer Metadata

When you place virtual machine or virtual machine set instances behind an Azure Standard Load Balancer, you can use IMDS to retrieve metadata related to the load balancer and the instances. For more information, see [Retrieve load balancer information](../load-balancer/instance-metadata-service-load-balancer).

## Scheduled events

You can obtain the status of the scheduled events by using IMDS. Then the user can specify a set of actions to run upon these events. For more information, see [Scheduled events for Linux](linux/scheduled-events) or [Scheduled events for Windows](windows/scheduled-events).
...
* **I created my VM through Azure Resource Manager some time ago. Why am I not seeing compute metadata information?**
	...
	+ If you created your VM after September 2016, add a [tag](../azure-resource-manager/management/tag-resources) to start seeing compute metadata. If you created your VM before September 2016, add or remove extensions or data disks to the VM instance to refresh metadata.
* **Is user data the same as custom data?**

	+ User data offers the similar functionality to custom data, allowing you to pass your own metadata to the VM instance. The difference is, user data is retrieved through IMDS, and is persistent throughout the lifetime of the VM instance. Existing custom data feature will continue to work as described in [this article](custom-data). However you can only get custom data through local system folder, not through IMDS.
...
* **Why am I getting the error `500 Internal Server Error` or `410 Resource Gone`?**
	...
	+ Retry your request. For more information, see [Transient fault handling](/en-us/azure/architecture/best-practices/transient-faults). If the problem persists, create a support issue in the Azure portal for the VM.
...
* **Why is my request timed out for my call to the service?**
	...
	+ Metadata calls must be made from the primary IP address assigned to the primary network card of the VM. Additionally, if you've changed your routes, there must be a route for the 169.254.169.254/32 address in your VM's local routing table.
		...
		- [Windows](#tabpanel_15_windows)
		- [Linux](#tabpanel_15_linux)
...
* [Acquire an access token for the VM](../active-directory/managed-identities-azure-resources/how-to-use-vm-token)
* [Scheduled events for Linux](linux/scheduled-events)
* [Scheduled events for Windows](windows/scheduled-events)
