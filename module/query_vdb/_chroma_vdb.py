...
def get_chroma_vdb_retriever(_db_name):
...
def qa_chroma_retriever_vdb(_query, _db_name):
    from langchain.chat_models import ChatOpenAI
    from dotenv import load_dotenv
    load_dotenv()
    import os
    llm = ChatOpenAI(model_name=os.getenv('OPENAI_MODEL'), temperature=0)
    _retriever = get_chroma_vdb_retriever(_db_name)
...
def qa_chroma_vdb(_query, _db):
    ...
    _ans = qa_chroma_retriever_vdb(_query, _db_name)
    return _ans

def qa_chroma_vdb_azure(_query):
    _ans, _steps = "", ""
    _ans = qa_chroma_vdb(_query, "azure_virtual_machines_plus")
    return [_ans, _steps]

def qa_chroma_vdb_langchain(_query):
    _ans, _steps = "", ""
    _ans = qa_chroma_vdb(_query, "langchain_python_documents")
    return [_ans, _steps]
